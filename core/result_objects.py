...
POSITION RESULTS:
    PositionResult provides the standard envelope for position data:
    module/version/status metadata + raw positions + summary, with strict
    validation via PositionsData to surface data issues early.

POSITION MONITOR:
    PositionResult also provides a monitor view with exposure and P&L metrics.
    The monitor view:
...
class RealizedPerformanceResult:
    ...
    def _categorize_performance(self) -> str:
        ...
        if sharpe >= 1.5 and annual_return >= 0.15:
            return "excellent"
        if sharpe >= 1.0 and annual_return >= 0.10:
            return "good"
        if sharpe >= 0.5 and annual_return >= 0.05:
            return "fair"
        return "poor"
