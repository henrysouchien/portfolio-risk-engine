...
from portfolio_risk_engine.risk_summary import (
...
from core.proxy_builder import inject_all_proxies
from utils.gpt_helpers import (
...
from portfolio_risk_engine.scenario_analysis import analyze_scenario
from portfolio_risk_engine.optimization import optimize_min_variance, optimize_max_return
...
from portfolio_risk_engine.stock_analysis import analyze_stock
from portfolio_risk_engine.performance_analysis import analyze_performance
...
def run_portfolio(
    ...
    *,
    ...
    if return_data:
        ...
        from core.run_portfolio_risk import display_portfolio_config
        if isinstance(filepath, str):
            config = load_portfolio_config(filepath)
        else:
...
def _handle_new_tickers_for_cli(
    ...
    try:
        ...
        resolved_filepath = resolve_config_path(filepath)
        ...
        resolved_scenario_yaml = resolve_config_path(scenario_yaml) if scenario_yaml else None
        if resolved_scenario_yaml and resolved_scenario_yaml.exists():
            # Parse scenario YAML file
            delta_dict, new_weights = parse_delta(yaml_path=str(resolved_scenario_yaml))
            if new_weights:
...
def run_portfolio_performance(filepath: str, *, return_data: bool = False, benchmark_ticker: str = "SPY") -> Union[None, PerformanceResult, Dict[str, Any]]:
    ...
    Calls into:
    - `portfolio_risk_engine.performance_analysis.analyze_performance`.
    ...
    Notes
    -----
    * Delegates to portfolio_risk_engine.performance_analysis.analyze_performance()
...
def run_realized_performance(
    ...
    *,
    ...
    source : str
        Transaction source filter: "all", "snaptrade", "plaid", "ibkr_flex",
        "ibkr_statement", "schwab", or "schwab_csv".
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source",
        choices=["all", "snaptrade", "plaid", "ibkr_flex", "ibkr_statement", "schwab", "schwab_csv"],
        default="all",
