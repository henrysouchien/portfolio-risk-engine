...
> 2. **A scoped autonomous grant for F129** — the surviving, still-**unbuilt** piece of F134's intent, redesigned for the F2j profile-class model. **RESOLVED (2026-05-26 F2j machinery investigation): a dedicated `monitor` profile.** `--profile` is already a per-run dispatch arg (`entry.py:1197`, `load_profile(args.profile)`) — NOT hardcoded to analyst — so the cadence driver dispatches `agent.autonomous --profile monitor --skill position-reassess`. Add `api/agent/profiles/monitor.py` (sibling to `analyst.py`/`advisor.py`) via the F2j profile-composition machinery. **Scope tightly via `allowed_extra_tools`, NOT whole classes:** `allowed_classes = {read, pure_transform}` + `allowed_extra_tools = {apply_patch_ops, thesis_append_decisions_log, update_watch_item, notify_send}` (4 tools, each with a `# rationale:` comment per F2j §3.4 lint). This grants ONLY F129's exact tools — it does NOT admit the whole `state_write`/`external_write` classes (which would pull in `set_target_allocation`, `create_portfolio`, `gsheet_*`, etc.). `irreversible` stays absent → passes the F2j config-load assertion. The rejected alternative — a NEW per-skill class-grant in skill frontmatter — is net-new infra (skills cannot select a profile or declare classes today; confirmed) and unnecessary given the `--profile` path exists. Granting anything to the **analyst** profile wholesale remains unacceptable (opens every autonomous analyst run). **Caveat to validate in review:** F2j classifies `apply_patch_ops` as a single class (not input-aware), so granting it admits model-bound ops too; F129 only emits monitoring ops (D-G), so acceptable, but note the input-aware distinction F134 wanted isn't present.
> 3. F134 as originally planned is **NOT** a prerequisite — its classification half shipped via F2j; its gate half doesn't match the runtime; only the scoped-grant capability (item 2) survives, re-homed here.
>
> **Sequencing now:** F2j PR1+PR2 (done) → F125 mutation-aware validation → F129 PR0 / PR1 / PR2 (PR2 creates+classifies `update_watch_item`) → PR-A (`monitor` profile, must follow PR2) → PR3+. This replaces the old "F125 → F134 → F129" chain. (Full PR ordering + dependencies in §5.)
...
- Frontmatter: `scope: ticker`, `state_class: producer`, `agent_callable: true`, `resumable: true`, `persist_state: true`, `max_turns: 12`, `max_budget_usd: 2.0`, `mcp_servers: [research-mcp, portfolio-mcp, fmp-mcp, alerts]`. **`research-mcp` is required** — `thesis_read`/`thesis_list` live there, NOT on portfolio-mcp (Codex round 1). `portfolio-mcp` provides `apply_patch_ops`, `check_exit_signals`, `get_quote`, `get_positions`, `evaluate_thesis_monitoring`. All four must be in the cron template + (for `research-mcp`/`portfolio-mcp`) the `USER_EMAIL_REQUIRED_MCP_SERVERS` set, which already lists both.
...
Prerequisites: **F125** (mutation-aware audit validation, for PR2) + **PR-A** (the `monitor` profile). F134-as-planned is NOT a prerequisite (see reconciliation banner). Ordering note: **PR-A must come after PR2** — the `monitor` profile's `allowed_extra_tools` can only list tools that exist *and are classified in `server_policies.py`*, and `update_watch_item` is created+classified in PR2 (F2j rejects unknown `allowed_extra_tools` at profile load, `profiles/__init__.py:100`). Within F129:
...
| **PR-A** | **Dedicated `monitor` profile** (`api/agent/profiles/monitor.py`, sibling to analyst/advisor; an AI-excel-addin F2j profile-layer prerequisite that F129 *consumes*): `allowed_classes = {read, pure_transform}` + `allowed_extra_tools = {apply_patch_ops, thesis_append_decisions_log, update_watch_item, notify_send}` (each with `# rationale:` comment per F2j §3.4 — 4 entries is fine; the ~3 cap is a soft review signal, not a hard lint). Cadence driver dispatches `--profile monitor` (the `--profile` path already exists — `entry.py:1197`/`:1221`; no `run_skill` change). MUST NOT touch the analyst profile. `irreversible` absent → F2j config-load assertion passes. Tests: `monitor` grants exactly the 4 tools + nothing else; `execute_trade`/other `irreversible`/`set_target_allocation` still excluded; config-load assertion holds. | AI-excel-addin (F2j profile layer) | **PR2** (needs `update_watch_item` registered+classified before the profile can list it) |
...
| PR4 | `alerts` server in both cron templates + headless-safe `notify_send` path (alerts MCP) + gate tests (`notify_send` is already classified `external_write`, `server_policies.py:217`) | AI-excel-addin + alerts MCP | **PR-A** (`monitor` profile grants `notify_send`) |
...
| PR6 | Cadence scheduling wrapper (`thesis_list ∩ get_positions`, per-`research_file_id` fan-out, dispatch `--profile monitor`) + live dry-run | AI-excel-addin / risk_module | PR5 |
