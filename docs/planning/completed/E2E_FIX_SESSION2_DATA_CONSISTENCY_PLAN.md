...
**Status**: IMPLEMENTED — 5/7 findings fixed, R7 deferred to investigation
**Session**: 2 of 4 (parallel)
**Commits**: `8531a6f3` (R6, R8, R12, R15, R21 fixes + tests)
...
| Fix | Finding | Status | Live Result |
|-----|---------|--------|-------------|
| R6 | Weight denominator | ✅ VERIFIED | Weights use `total_portfolio_value` (net+cash). DSU 28.7% = $14.6K/$51K NAV |
| R12 | Holdings count | ✅ VERIFIED | "View All 19 Holdings" matches backend filtered count |
| R15 | Volatility | ✅ VERIFIED | Performance strip: 8.33% (realized). IBKR view: 16.25% (forward-looking). No more 19.4% |
| R8+R22 | Margin dedup | ✅ DEPLOYED | Cash dedup by (ticker, brokerage, canonical_account). Needs raw data verification |
| R21 | Allocation gap | ✅ VERIFIED | Total Allocation: 100.0%. All categories present including Cash (Margin) |
| R7 | Total value ($131K > $109K) | DEFERRED | Needs debug endpoint + live data investigation |

Tests: 1807 passed, 0 regressions. 7 new test cases added.
