...
**Status:** ✅ **SHIPPED + LIVE-VERIFIED 2026-05-29.** PR0a–PR6 + research_producer manifest entry shipped across risk_module + AI-excel-addin + alerts-mcp (10 path-scoped commits). Live-verified end-to-end on PCTY thesis 225 (live-headless mode, 3:40 min, exit 0, `INSUFFICIENT_DATA` verdict per spec). Closeout in [`docs/TODO_COMPLETED.md`](../../TODO_COMPLETED.md) (Docs / Queue Hygiene → 2026-05-29 F129). Reference doc: [`docs/architecture/AUTONOMOUS_MONITORING_LOOP.md`](../../architecture/AUTONOMOUS_MONITORING_LOOP.md) — the canonical "how it works" doc; this plan is preserved as build history.

**Build history (was: PLAN — CODEX PASS round 3 + delta + research_producer convergence delta).** F125 mutation-aware + carry-forward (`7b6b9281`/`3a77105c`) and `research_producer` profile (F131 enablement Part 1+2, `cfd8ef4e`/`c4d268dc`) were the unblocking prereqs — both shipped before F129 PR sequence executed.
...
**Follow-ups filed:** F163 (papercuts — agent_description over cap + missing PRODUCT_ID in systemd unit). F132 (rich PositionSnapshot expansion) parked. F131 live audit gate still pending its first live green run on shipped research_producer.
...
>
> **Sequencing now:** F2j PR1+PR2 ✅ + F125 mutation-aware validation ✅ + F131-enablement Part 1+2 (`portfolio_config` split + `research_producer` profile) ✅ → **F129 PR0 / PR1 / PR2 / PR-A / PR3 / PR4 / PR5 / PR6** (all internal to F129; no remaining external deps). (Full PR ordering in §5.)
...
- [`F134_AUTONOMOUS_WRITE_ENFORCEMENT_PLAN.md`](completed/F134_AUTONOMOUS_WRITE_ENFORCEMENT_PLAN.md) + [`F126_AUTONOMOUS_WRITE_GATING_POLICY.md`](completed/F126_AUTONOMOUS_WRITE_GATING_POLICY.md) — write classification (`thesis_only` / `operational` / `portfolio_affecting`)
...
- [`F124_THESIS_WATCHLIST_OWNERSHIP_WIRING_PLAN.md`](completed/F124_THESIS_WATCHLIST_OWNERSHIP_WIRING_PLAN.md) — the typed `WatchItem` / `Monitoring` / `update_monitoring` substrate F129 consumes
...
- `schema/handoff_patch.py`: `class UpdateWatchItemOp(_PatchOpBase)` with `op: Literal["update_watch_item"]`, `target: {watch_item_id: str}`, `value: WatchItemWriteValue` (partial: at minimum `last_checked`; optionally `description`/`metric`/`threshold`/`threshold_direction`/`source_refs`). **NOT writable via this op:** `derived_from_handle` / `derived_from_kind` (set at watch-item creation by `monitoring-init`; never mutated after — see §"WatchItem F125 fields" below).
- `api/research/patch_engine.py`: `_apply_update_watch_item_patch` — locate watch item by `watch_item_id`, apply field updates, **preserve untouched fields — including `source_refs`, `derived_from_handle`, and `derived_from_kind`** (the F125 carry-forward fields added 2026-05-28; dropping them breaks `extra="forbid"` AND loses the carry-forward authorization that lets `last_checked`-only updates pass `validate_mutation`). Descriptor + within-batch conflict detection (mirror `_apply_monitoring_patch` from F124).
- **WatchItem F125 fields (added in F125 PR-B `3a77105c`):** `WatchItem` now has `derived_from_handle: str | None` + `derived_from_kind: DerivedFromKind | None` (`thesis_shared_slice.py:382-383`). `monitoring-init` sets these on every watch item (parent `catalyst_id`/`trigger_id` + kind `catalyst`/`trigger`) so carry-forward inherits the parent's excerpt-linked evidence rather than requiring the watch item to carry its own excerpt. `f125_carry_forward.py:71` recognizes `WatchItem` specifically as a known carry-forward child; `patch_engine.py:457` auto-invokes `validate_mutation` during apply. F129's `last_checked`-only update is a neutral-metadata write (doesn't change any claim content) and is doubly safe: (1) mutation-aware validation only validates touched-claim content, and `last_checked` is metadata; (2) the watch item also has carry-forward authorization, so even strict claim re-validation would pass. **Implication:** F129's PR2 `_apply_update_watch_item_patch` MUST round-trip `derived_from_handle`/`derived_from_kind` unchanged on every update path.
...
- `update_watch_item`: `last_checked`-only = R1-exempt success **even when the thesis has unrelated historical unsourced claims** (proves F125 mutation-aware validation); claim-field change without `source_refs` rejected; unknown id → `InvalidTargetError`; same-batch dup → conflict; untouched `source_refs` preserved; **`derived_from_handle` + `derived_from_kind` round-tripped unchanged on every update path** (F125 carry-forward authorization preserved; dropping them would break `extra="forbid"`).
