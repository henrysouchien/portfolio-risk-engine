...
20-fix progression from Feb 25 to Mar 8, 2026. Started at 100+pp off across all providers, ended with all major accounts within ~1pp of broker actuals.

| Account | Engine | Broker | Gap | Status |
|---------|--------|--------|-----|--------|
| IBKR (U2471778) | -0.19% | +0.29% | **0.48pp** | ✅ Solved |
| Plaid (Merrill) | -11.36% | -12.49% | **1.13pp** | ✅ Solved |
| Schwab 165 | -7.35% | -8.29% | **0.94pp** | ✅ Solved |
| Schwab 252 | +13.13% | +10.65% | **2.48pp** | ⚠️ Structural (TWR vs XIRR + FMP vs Schwab pricing) |
| Schwab 013 | +2.36% | -14.69% | **17pp** | ⚠️ Low priority ($2K account, 2% weight) |

**Schwab 252 structural gap**: All hypotheses investigated — FMP uses unadjusted `close` (ruled out), ENB DRIP NAV-neutral (ruled out). Gap is TWR vs XIRR methodology + $1,351 (2.5%) FMP vs Schwab position pricing difference. Not fixable without switching pricing sources.

See `performance-actual-2025/RETURN_PROGRESSION_BY_FIX.md` for full 20-fix progression and account 252 deep-dive.

## Original Problem (2026-02-25)

`get_performance(mode="realized", format="agent")` showed poor data quality across all providers. Coverage was low, most positions were synthetic (inferred from current holdings with no opening trade), and return metrics were unreliable. The system produced returns 100pp off in the wrong direction.
...
**Complete.** All three original distortion sources (synthetic cash inflation, futures notional amplification, Plaid phantom volume) resolved across 20 fixes. Major accounts within ~1pp of broker actuals. Remaining gaps are structural (methodology/pricing differences) or low-priority (tiny accounts).
