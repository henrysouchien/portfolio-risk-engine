...
**Status**: COMPLETE. Commit `af30d415`. Cross-source exclusion bypassed for Merrill — all 4 symbols (IT, DSU, MSCI, STWD) now included.
...
| Source | TWR | Broker | Gap | Status |
|--------|-----|--------|-----|--------|
| Schwab | +5.72% | — | — | Unchanged (3-account aggregation) |
| IBKR | -11.37% | -9.35% | 2.0pp | Unchanged (auto-resolves institution, 1 account fallback) |
| Plaid/Merrill (old, no institution) | -10.46% | -12.49% | 2.0pp | Cross-source exclusion hides 3/4 symbols |
| Plaid/Merrill (new, institution=merrill) | -9.04% | -12.49% | 3.4pp | All 4 symbols included, no cross-source exclusion |

Cross-source exclusion confirmed bypassed:
- Without account filter: `source_holding_symbols: ['IT']`, `cross_source_leakage: ['DSU', 'MSCI', 'STWD']`
- With `institution='merrill'`: `source_holding_symbols: ['DSU', 'IT', 'MSCI', 'STWD']`, `cross_source_leakage: []`
- With `account='CMA-Edge'`: same as institution-only (account filter runs before cross-source detection)

Remaining 3.4pp gap vs broker is a separate issue (transaction timing, Plaid data completeness, etc.) — not caused by cross-source exclusion.

Tests: 1296 passed, 1 pre-existing date-sensitive failure, 31 skipped.
