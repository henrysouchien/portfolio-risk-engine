...
- A stub root `brokerage/__init__.py` reappeared during the run from a parallel session and was left untouched per shared-workspace rule. Sys.path resolution went to `brokerage-connect/brokerage/` (correct) because the editable install pth came first. If the stub's presence is unintentional, file separately.
