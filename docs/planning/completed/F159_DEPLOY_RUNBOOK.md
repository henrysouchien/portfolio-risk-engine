...
**Plan**: `docs/planning/completed/F159_CORPUS_PATH_RESOLVE_PLAN.md` (Codex R4 PASS)
...
- Step 9 split: `--dry-run` stays as F159 unblock proof; `--execute` deferred to separate F160 session
...
## Step 9 — Unblock F160 round-trip (dry-run proof only)

R2: split — only the dry-run lives in F159's deploy window. The `--execute` + bulk refresh + promote (30-90 min) is deferred to a separate F160 session outside this prod repair rollback window.
...
**Do NOT run `--execute` here.** That's a separate F160 session.
