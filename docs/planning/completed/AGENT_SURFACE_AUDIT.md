> **✅ DONE — Audit workstreams shipped or explicitly deferred; moved during 2026-05-28 docs cleanup.**
...
| Phase | Scope | Status |
|---|---|---|
| **Phase 1** | Type-polish `risk_client` (typed sigs + curated TypedDicts) | ✅ **SHIPPED 2026-04-17** — see `completed/RISK_CLIENT_TYPE_POLISH_PLAN.md` ship log |
| **Phase 2** | Extract pure-compute math kernels into sandbox-local `portfolio_math/` package | ✅ **SHIPPED 2026-04-17** — see `completed/PORTFOLIO_MATH_EXTRACTION_PLAN.md` ship log. 11 MVP kernels. |
| **PM1A** | AI-excel-addin subprocess mount for `portfolio_math` | ✅ **SHIPPED 2026-04-19** — see `completed/PORTFOLIO_MATH_SANDBOX_MOUNT_PLAN.md` ship log. Agent sandbox can now `import portfolio_math`. |
| **Phase 3B** | `building_blocks` delegate to `portfolio_math` directly (non-breaking) | ✅ **SHIPPED 2026-04-21** — see `completed/BUILDING_BLOCKS_INTERNAL_DELEGATE_PLAN.md` ship log (`afe4fe93`). |
| **Phase 3C** | Monte Carlo kernel extraction | ✅ **SHIPPED 2026-04-26 (end-to-end)**. (a) risk_module extraction per `completed/PORTFOLIO_MATH_MONTE_CARLO_EXTRACTION_PLAN.md` (v6, Codex R6 PASS after 6 review rounds; squash commit `0ef688fd`/PR #21): two-tier API at `portfolio_math` root — low-level `simulate_paths(covariance, weights, drift, ...)` for sandbox composition + high-level `run_monte_carlo_from_components(...)` mirroring today's behavior — plus 4 building-block primitives (`compute_monthly_drift_from_components`, `build_covariance_transform`, `nearest_psd`, `terminal_stats`). Engine wrapper preserves wire format; 7 private helpers become thin import-shims. Algorithm preserved verbatim — vol_scale-on-covariance, bootstrap fallback, scenario_shocks nonlinear monthly↔annual round-tripping all byte-identical via `tests/fixtures/monte_carlo_snapshot.json` regression test. (b) AI-excel-addin cross-repo per `completed/PORTFOLIO_MATH_MONTE_CARLO_AI_EXCEL_ADDIN_PLAN.md` (v2, Codex R2 PASS; squash commit `61d8dbb9`/PR #49): subprocess smoke test pinning `pm.simulate_paths(seed=42)` to `mean ≈ 108.631818` + `probability_of_loss == 0.43` + system prompt enumerates the 6 MC primitives. Agent sandbox can now `import portfolio_math as pm`, compose `pm.simulate_paths(...)` with custom drift, and chain MC outputs into other portfolio_math primitives without HTTP round-trips. (c) Live agent validation 2026-04-27: agent discovered `pm.simulate_paths` from prompt, picked `host="subprocess"`, threaded kwargs correctly. One prompt gap caught + fixed (AI-excel-addin PR #50, squash `b537964`) — agent assumed dataclass return; appended return-shape + bracket-access example to the MC bullet. (d) Re-validated post-PR #50: agent now uses `r["terminal_distribution"]["mean"]` zero-shot; full round-trip succeeds end-to-end with numerics byte-identical to addin smoke test pin (`mean=108.63181760586698`, `pol=0.43`). |
| **Phase 3D** | Option payoff + `OptionLeg`/`OptionStrategy` domain type extraction | ✅ **SHIPPED 2026-04-22 (end-to-end)** — (a) risk_module extraction per `completed/PORTFOLIO_MATH_OPTIONS_PAYOFF_EXTRACTION_PLAN.md`: `portfolio_math` exports `OptionLeg`, `OptionStrategy`, and all 10 pure payoff functions at root (un-curated per `completed/PORTFOLIO_MATH_OPTIONS_ROOT_EXPORT_FIX_PLAN.md`), matching Phase 2's pricing-primitive pattern, with legacy `options.*` paths preserved via shims. (b) AI-excel-addin cross-repo per `completed/PORTFOLIO_MATH_OPTIONS_AI_EXCEL_ADDIN_PLAN.md`: subprocess smoke test + system prompt enumerates the full root surface for agent discovery. Agent sandbox can now `import portfolio_math as pm`, compose `pm.OptionStrategy`, and run full payoff analysis locally with zero HTTP round-trips. |
| **Phase 3E** | Agent API multi-user auth via gateway-signed user claim | ✅ **SHIPPED 2026-04-22** — replaced the static sandbox bearer model with gateway-signed HMAC-SHA256 user claims across both repos. risk_module shipped the verifier + dual-mode `risk_client` (`2fe96075`, `6e9cc689`) and `/chat/init` user_email threading (`6d2a131f`); AI-excel-addin shipped `/chat/init` accept/persist (`5099f18`), signer + sandbox env denylist (`b16e8e2`), and registry fetcher migration (`985d30a`). Plan: `completed/AGENT_API_SIGNED_USER_CLAIM_PLAN.md`. |
