...
**Status**: SHIPPED 2026-04-22 — risk_module extraction plus AI-excel-addin surface completion
**Date**: 2026-04-21
**Predecessor**: `docs/planning/completed/PORTFOLIO_MATH_EXTRACTION_PLAN.md` (Phase 2, shipped 2026-04-17), `docs/planning/completed/BUILDING_BLOCKS_INTERNAL_DELEGATE_PLAN.md` (Phase 3B, shipped 2026-04-21)
...
4. **Recommend #4 (curate root payoff exports)** → Accepted at ship time. Reversed 2026-04-22 by `completed/PORTFOLIO_MATH_OPTIONS_ROOT_EXPORT_FIX_PLAN.md`; see §13. Only `leg_payoff`, `strategy_payoff`, `find_breakevens`, `payoff_table` + `OptionLeg`, `OptionStrategy` shipped at root in v3. The other 6 payoff functions were later promoted to `portfolio_math.*` root to match Phase 2's un-curated pricing-primitive pattern.
...
`completed/PORTFOLIO_MATH_OPTIONS_ROOT_EXPORT_FIX_PLAN.md` reversed the curated root-export decision from Step 3 and promoted the remaining 6 payoff functions (`max_profit`, `max_loss`, `intrinsic_value`, `extrinsic_value`, `cost_of_leverage_annualized`, `pnl_per_dollar_move`) to `portfolio_math.*` root.
