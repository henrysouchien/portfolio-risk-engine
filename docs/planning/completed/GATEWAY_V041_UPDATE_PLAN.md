...
**3a. DEPLOY_CHECKLIST.md**: Add version compatibility note under the agent-gateway section stating that the `app-platform` gateway proxy is version-agnostic (byte passthrough) and does not need updates when the upstream gateway version changes.
