...
### March 23 Follow-Up: Share Realized Attribution Input Prep

One remaining realized-performance debt seam was duplicated attribution-input preparation. The stale startup path could rebuild the same weight/date/request-input payload more than once across:

- returns-dataframe prewarm
- attribution prewarm
- the later route-time attribution build

That duplicated the same fallback logic, including the expensive `reprice_position_values` branch when stale cached holdings did not already have market values.

This follow-up added a shared short-lived realized-attribution-input snapshot keyed to the same realized request shape as the existing returns/attribution caches. The result is narrower but cleaner:

- stale repricing and weight assembly now happen once per request shape inside the snapshot TTL window
- returns-dataframe prewarm and later attribution build can attach to the same prepared inputs instead of rebuilding them independently
- attribution math and API response shape stay unchanged

Verification for this slice:

- `python3 -m pytest tests/services/test_performance_helpers.py tests/services/test_portfolio_result_cache.py -q`
- `25 passed`
- `python3 -m pytest tests/routes/test_realized_performance_route.py tests/routes/test_auth_route.py tests/routes/test_onboarding.py -k 'realized or dashboard_prewarm or auth_status or prime_virtual_portfolio' -q`
- `9 passed`
...
## Portfolio-Switch Audit

March 23, 2026 follow-up on the remaining dashboard state/cache risk found one real selected-portfolio leak in the overview stack:

- `market-intelligence` and `metric-insights` were treated as effectively global on the frontend resolver side, so query keys did not distinguish the selected portfolio
- `useAIRecommendations()` also used a global query key, so switching portfolios could reuse the prior portfolio's recommendations until a hard invalidation happened
- the backend short-lived overview caches for market intelligence, AI recommendations, and metric insights were keyed only by `user_id`, not by `portfolio_name`
- `build_metric_insights()` was still hardcoded to `CURRENT_PORTFOLIO`, so even a clean refetch could analyze the wrong portfolio after a switch

This slice fixed the scope boundary end-to-end:

- frontend overview hooks now key portfolio-specific requests by `portfolioId`
- resolver/API calls now pass the resolved `portfolio_name` through for market intelligence and metric insights
- AI recommendations now key by `portfolioId` and request the resolved `portfolio_name`
- backend overview result caches now distinguish `portfolio_name`
- metric insights now analyzes the requested portfolio instead of hardcoded `CURRENT_PORTFOLIO`
- market-intelligence symbol/weight loading now supports selected named portfolios through the workflow portfolio snapshot path instead of only the implicit consolidated portfolio path

Focused verification for this slice:

- frontend: **41 passed** across resolver + overview-hook scoping tests
- backend: **64 passed** across result-cache + metric-insights + factor-intelligence + news-events tests
- frontend typecheck: **clean**

## March 23, 2026 Frontend Request-Topology Follow-Up

After the backend hot paths were largely back under control, I re-ran the default `#score` dashboard load in a real browser to look for duplicate or optional first-load requests.

Initial browser request burst findings on the live dev app:

- `POST /api/performance/realized`: **3** calls
- `GET /api/positions/holdings?portfolio_name=CURRENT_PORTFOLIO`: **3** calls
- `GET /api/risk-settings?portfolio_name=CURRENT_PORTFOLIO`: **1** call even though the score view does not use risk settings

The causes were split across two frontend layers:

- the score view mounted overlapping `usePortfolioSummary()`, `usePerformance()`, and `usePositions()` hooks across the overview container, performance strip, holdings card, and earnings card
- the `performance` resolver still called `realized-performance` directly instead of going back through nested-source query reuse
- the eager data-source scheduler was prefetching `risk-profile`, which hits `/api/risk-settings`, even though nothing on the score view consumes that source

This slice tightened the default dashboard path:

- the score view now shares one summary hook, one performance hook, and one positions hook across the overview cards instead of mounting redundant copies
- the `performance` resolver now reuses the nested `realized-performance` query path, so repeated default-performance loads can attach to the same in-flight/cached realized result
- `risk-profile` now loads lazily instead of being eagerly prefetched on score-view startup

Measured impact from the live browser burst after these changes:

- `POST /api/performance/realized`: **3 → 1**
- `GET /api/risk-settings?portfolio_name=CURRENT_PORTFOLIO`: **1 → 0**
- `GET /api/positions/holdings?portfolio_name=CURRENT_PORTFOLIO`: **3 → 2**

Current request-topology conclusion:

- the default score-view hot path no longer repeats realized performance work
- optional risk-settings work is no longer leaking onto the default dashboard load
- one extra holdings request still remains on the score view, along with duplicate auth/dev-login traffic in local dev, but the biggest avoidable first-load duplication is now removed

March 23, 2026 follow-up on the remaining startup burst closed the last clear duplicate-request regressions on the default `#score` path:

- `auth/status` + `dev-login` were still happening twice in local dev because auth bootstrap was not idempotent under React dev double-effects
- `smart-alerts` and `positions-enriched` were not treated as portfolio-scoped by `useDataSource()`, so they could issue one `{}` query before portfolio bootstrap and then immediately issue a second `{ portfolioId }` query after bootstrap
- the score cards still mounted a separate `usePositions()` query even though `portfolio-summary` had already paid for `positions-enriched` internally

This follow-up fixed those remaining owners:

- auth bootstrap is now in-flight deduped at the generic auth-store layer, so strict-mode double effects no longer duplicate `/auth/status` and `/auth/dev-login`
- `useDataSource()` now treats `positions-enriched` and `smart-alerts` as portfolio-scoped sources, so they stay idle until the active portfolio exists and then load only once
- the score-view holdings and earnings cards now reuse enriched holdings fields carried through `portfolio-summary` instead of mounting a second holdings query on top of the summary load
- nested `portfolio-summary` source resolution now routes through the same shared resolver path as mounted hooks, so direct and nested loads can join the same in-flight work

Measured impact from the latest live browser burst on `http://localhost:3000/#score`:

- `GET /auth/status`: **2 → 1**
- `POST /auth/dev-login`: **2 → 1**
- `GET /api/positions/alerts?portfolio_name=CURRENT_PORTFOLIO`: **2 → 1**
- `GET /api/positions/holdings?portfolio_name=CURRENT_PORTFOLIO`: **2 → 1**
- `POST /api/performance/realized`: stays at **1**
- `GET /api/risk-settings?portfolio_name=CURRENT_PORTFOLIO`: stays at **0**

Updated request-topology conclusion:

- the default score-view startup burst is now down to one auth check, one dev auto-login, one holdings load, one alerts load, and one realized-performance load
- the remaining user-visible startup work is now mostly legitimate data loading, not duplicate frontend request topology

## March 23, 2026 First Meaningful Score Paint

After the request-topology cleanup, I measured actual browser-visible `#score` readiness instead of just request counts.

Initial browser timing on the long-lived dev process showed the shell painting quickly but the dashboard staying visually incomplete much longer:

- browser shell paint stayed fast at roughly **350-450ms**
- warmed score-page “usable/full” load was roughly **2.06s to 2.36s**
- the worst cold run stretched to roughly **11.05s**, gated by a late `POST /api/performance/realized`

The root cause turned out to be structural, not just raw backend latency:

- the `portfolio-summary` resolver waited on `positions-enriched`, `risk-score`, `risk-analysis`, and `performance` before returning anything
- that meant the overview could stay in `Overview: Mock | Portfolio: Loaded` with `—` metrics and loading holdings even after `GET /api/portfolios/CURRENT_PORTFOLIO` had already completed
- the score page was effectively coupling first meaningful paint to the slower risk/performance path, even though the immediate user-visible need is the positions-backed overview

This slice changed the summary path to return a positions-backed summary first and backfill the slower enrichments into the same query after the nested sources complete:

- `portfolio-summary` now resolves `positions-enriched` immediately and transforms that into a real summary/holdings payload
- cached nested `risk-score`, `risk-analysis`, and `performance` data are still folded in synchronously when already warm
- missing nested sources are fetched in the background and then patch the same `portfolio-summary` query with the enriched result
- the summary adapter now preserves missing derived metrics as `null` instead of fabricating `0` values while those enrichments are still outstanding

Measured impact from a fresh browser pass on `http://localhost:3000/#score` after this change:

- cold-ish run:
  - overview becomes real at **2146ms**
  - holdings are ready at **2150ms**
  - full top fold is populated at **2575ms**
- warm runs:
  - overview becomes real at **1014ms** and **1023ms**
  - holdings are ready at **1039ms** and **1267ms**
  - full top fold is populated at **1133ms** and **1278ms**

Request ordering now matches the visible behavior more cleanly:

- `GET /api/portfolios/CURRENT_PORTFOLIO` and `GET /api/positions/holdings` are enough to unlock the real overview state
- `POST /api/performance/realized` can still be slow on a cold outlier, but it no longer keeps the overview stuck in mock/blank state while waiting

Live browser confirmation:

- at a 4-second snapshot, the score page already showed real overview metrics, loaded holdings, loaded alerts, asset allocation, and portfolio earnings
- this replaced the earlier behavior where the same mid-load snapshot could still show `Overview: Mock | Portfolio: Loaded`, `—` metrics, and `Loading holdings...`
- a Playwright regression guard now exists in `e2e/tests/score-dashboard-paint.spec.ts`; it deliberately delays realized-performance and asserts the score overview becomes real before that slow call resolves
...
- `portfolio-summary` now returns a positions-backed result before slow nested risk/performance enrichments finish, and then backfills the same query once they do
- the portfolio-summary adapter now preserves missing derived summary metrics as `null` until the enrichment data actually exists
- a Playwright score-dashboard paint regression test now protects the “real overview before slow performance completes” behavior
...
- [x] measure time to first meaningful dashboard paint
...
## March 23 Fix: Stale-Cache Position Repricing (`1e1a611b`)

Fresh measurement on **2026-03-23** found that single-account `POST /api/analyze` returned **500** ("No valid return data, All 0 tickers excluded") and all holdings showed `current_price: 0.0` on the stale startup path.

Root cause: two bugs in `services/position_service.py`:
1. `_should_reprice_cached_provider()` only checked `stale_fallback` metadata, but the `allow_stale_cache=True` path sets `stale_forced_fallback` and the grace-window path sets `stale_grace_fallback` — neither triggered repricing
2. The zero-value fallback guard checked `value.isna()` but DB-cached positions have `value=0.0` (not NaN)

Fix: check all three metadata keys + scope fallback to cached-provider rows. Codex-reviewed (2 rounds, PASS). Plan: `STALE_CACHE_REPRICING_FIX_PLAN.md`.

### Post-Fix Measurements (2026-03-23)

| Metric | Pre-Fix (2026-03-23) | Post-Fix | March 19 Baseline |
|--------|---------------------|----------|-------------------|
| Combined cold wall | 6,194ms | 7,963ms | 1,640ms |
| Combined warm wall p50 | 264ms | 168ms | 115ms |
| Combined warm total p50 | 951ms | 648ms | 115ms |
| Single cold wall | 1,908ms | 3,695ms | 1,569ms |
| Single warm wall p50 | 424ms | 414ms | 156ms |
| Single analyze status | **500 (0.3KB)** | **200 (58.7KB)** | 200 |
| Single analyze warm | 455ms | 118ms | 124ms |

Key outcomes:
- **Functional fix**: single-account analyze returns real data, holdings have non-zero prices
- **Warm paths recovered**: combined warm wall p50 back to 168ms (from 264ms pre-fix)
- **Cold paths still regressed** vs March 19 — these are separate issues from the repricing bug

### Current Timing Breakdown (post-fix)

From `/api/debug/timing?minutes=5`:

- `positions_holdings_workflow`: **4.6s avg** — cold `get_all_positions` still expensive
- `realized_performance_payload_build`: **2.6s avg**
- `income_dividend_data_fetch`: **1.4s avg**
- `core.portfolio_analysis.analyze_portfolio`: **1.2s avg**, p50 **265ms** — warm is fine, cold outliers pull avg up
- `api_analyze_workflow`: warm p50 **295ms**
- `api_risk_score_workflow`: warm p50 **29ms**

### Current Regression Owners

The remaining regressions vs March 19 are concentrated in:

1. **Cold `get_all_positions`** — still takes 4-8s on the first call after cache clear. The stale-tolerant path returns fast but the background strict refresh is expensive.
2. **Cold `positions_holdings_workflow`** — 4.6s avg, dominated by `enrich_positions_with_risk` which waits for live prices.
3. **Cold `realized_performance_payload_build`** — 2.6s avg, mostly `load_transaction_store` + `build_price_cache`.
4. **Single-account warm `positions/alerts`** — 365ms in best warm run, noisier than expected.
5. **Combined cold wall** — 8.0s, gated by the slowest parallel endpoint (holdings or income).

## March 23 Follow-Up: Prewarm Timing Investigation

Investigated the warm analyze path regression (`build_portfolio_view` p50 1,113ms vs March 19's 555ms). Findings:

- The dashboard prewarm **IS working** — after a fresh bootstrap with 8s wait, analyze hits are 85-125ms (cache hit)
- Prewarm takes ~3-4s after bootstrap to complete the full analyze cache warmup
- The measurement script fires the overview burst immediately after bootstrap, before prewarm finishes
- The `build_portfolio_view` compute cost for 27 equities at ~1.1s is in the expected range for cold monthly-returns + factor analysis
- March 19's faster 555ms likely reflected warmer FMP price caches or fewer positions at that time
- No code-level regression found in `portfolio_risk.py` since March 19

Conclusion: the warm analyze path is healthy once prewarm completes. The apparent regression in the measurement script is a timing artifact — the script doesn't wait for background prewarm. The real user experience depends on browser bootstrap time (~1-2s) which gives prewarm a head start.

## March 23 Investigation: Cold Holdings / `get_all_positions`

Cold holdings takes **5.1s** (then 185ms warm, 79ms fully warm). The cost is NOT in position loading — with the repricing fix, stale-tolerant loading is fast. The cost is in **`enrich_positions_with_risk`**, which runs a full `portfolio_svc.get_positions_risk_result()` analysis to get per-position risk scores, beta, volatility, etc.

`get_positions_risk_result()` goes through `get_analysis_result_snapshot()` — the same cache as the dashboard prewarm. So in theory, the holdings risk enrichment should be a cache hit if prewarm has finished. But:

1. The holdings workflow starts the risk future at line 415 in a `ThreadPoolExecutor` immediately, before the prewarm has finished (prewarm takes ~3-4s)
2. The risk enrichment builds its own `analysis_portfolio_data` from the position result (lines 1442-1455) and calls `refresh_cache_key()` — this generates a different cache key than the prewarm because `standardized_input` differs (stale positions vs prewarm-derived positions)
3. Result: cache miss → full 1-2s analysis recompute inside the holdings workflow

### Root Cause: Cache Key Mismatch

The prewarm builds `analysis_portfolio_data` from `get_portfolio_snapshot()` + `_prime_virtual_portfolio_from_cached_positions()`. The holdings workflow builds its own from `get_portfolio_snapshot()` at line 1459 but also has a position-derived fallback. If the `standardized_input` differs between these two paths (due to stale vs fresh positions, or priming differences), the `PortfolioData.get_cache_key()` produces different hashes.

### Fix Direction

The holdings risk enrichment should reuse the prewarm's analysis result instead of computing its own. Options:
1. **Peek the prewarm's in-flight/cached analysis result** before starting a new risk future
2. **Defer risk enrichment** to after prewarm completes (non-blocking)
3. **Normalize the cache key** so both paths produce the same key for the same portfolio

## March 23 Fix: Non-Blocking Holdings Risk Enrichment (`b3f1553f`)

Replaced the blocking `ThreadPoolExecutor` + `risk_future.result()` pattern in `_load_enriched_positions()` with a non-blocking exact-key cache peek. Codex-reviewed (4 rounds, PASS). Plan: `HOLDINGS_RISK_ENRICHMENT_DEFERRED_PLAN.md`.

Changes:
- `peek_analysis_result_snapshot()` in `result_cache.py` — exact-key cache-only lookup, never blocks
- `get_positions_risk_result(non_blocking=True)` — returns None if analysis not cached
- `enrich_positions_with_risk(skip_fallback=True)` — prevents sync fallback to `get_positions_risk_result()`
- `_holdings_cache` invalidation when `_analysis_cache` is populated (prewarm or `/api/analyze`)
- Risk fields null on first cold hit, populated on next request after prewarm completes

Results: cold holdings 5-10s → 1.5-3s. Warm path unaffected.

## March 23 Investigation: Single-Account Warm Alerts

The 365ms warm alerts in the burst measurement was a timing artifact — alerts fired before prewarm/position caches were warm. After prewarm completes, single-account alerts are **21ms** steady-state. Not a bug.

## March 23 Investigation: Warm Path Steady-State

After full prewarm, per-endpoint warm timings are **significantly better** than March 19:

| Endpoint | Warm (now) | March 19 |
|----------|-----------|----------|
| holdings | 6ms | 11ms |
| alerts | 12ms | 12ms |
| analyze | 77ms | 176ms |
| risk-score | 14ms | 109ms |
| realized | 11ms | 71ms |
| target | 5ms | 109ms |
| income | 14ms | 61ms |
| market-intelligence | 3ms | 72ms |
| ai-recommendations | 5ms | 72ms |
| **Total** | **~147ms** | **~682ms** |

The measurement script warm numbers appear worse because the burst fires before prewarm completes — the first burst run pays a one-time re-cache cost. Steady-state warm is healthy.

## March 23 Investigation: Cold Path Floor

Cold path analysis on a fresh process:

| Step | Time | Notes |
|------|------|-------|
| Bootstrap | ~800ms | `get_portfolio_snapshot` + factor proxies |
| `get_all_positions` | ~1-2s | Provider fetch + repricing for 30 positions |
| `build_portfolio_view` | ~1.1s | Monthly returns for 27 equities |
| `income_dividend_data_fetch` | ~900ms | FMP dividend lookups for 30 tickers |
| `realized_performance_payload_build` | ~2s | Transaction store + price cache |

Cold parallel wall is **3.3s**, gated by the slowest parallel endpoint. The prewarm covers all of this within 3-4s, so the user experience after bootstrap + prewarm is fully warm (147ms total).

The cold path is at its practical floor given the current architecture. The remaining cost is inherent cold compute: FMP quote lookups, monthly returns computation, transaction store loading. Further cold-path optimization would require architectural changes (e.g., persistent warm caches across server restarts, pre-computed monthly returns).
...
1. **`1e1a611b` — Stale-cache position repricing** (Codex-reviewed, 2 rounds)
   - Fixed: single-account analyze 500 error, zero-price holdings
   - Fixed: metadata key mismatch in `_should_reprice_cached_provider()`
   - Fixed: zero-value fallback guard scoped to cached providers

2. **`b3f1553f` — Non-blocking holdings risk enrichment** (Codex-reviewed, 4 rounds)
   - Fixed: cold holdings 5-10s → 1.5-3s
   - Replaced blocking `ThreadPoolExecutor` + `risk_future.result()` with exact-key cache peek
   - Added `_holdings_cache` invalidation on analysis cache write

### Final Numbers vs March 19 Baseline

| Metric | March 19 | Final (2026-03-23) | Status |
|--------|----------|-------------------|--------|
| Combined warm wall p50 | 115ms | **153ms** (burst) / **~50ms** (steady) | Healthy |
| Single warm wall p50 | 156ms | **181ms** (burst) / **~50ms** (steady) | Healthy |
| Combined cold wall | 1,640ms | **3,333ms** | Acceptable (prewarm covers in 3-4s) |
| Single cold wall | 1,569ms | **2,706ms** | Acceptable (prewarm covers in 3-4s) |
| Single analyze | 500 error | **200 (58.7KB)** | Fixed |
| Warm endpoint total | ~682ms | **~147ms** | Better than baseline |

### Remaining Known Items (not blocking)

- Cold wall 2x March 19 — inherent compute for 30 positions (was fewer in March 19)
- `realized_performance_payload_build` cold 2s — transaction store loading, could be prewarmed earlier
- Measurement script burst numbers overstate warm regression due to timing artifact (fires before prewarm completes)
