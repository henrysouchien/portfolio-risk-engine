...
- [x] Document `providers/bootstrap.py` and `get_registry()` pattern in `docs/reference/` (`docs/reference/PRICING_PROVIDER_REGISTRY.md`)
- [x] Add integration test: register custom provider, verify it handles pricing for all analysis paths (`tests/providers/test_provider_integration.py`)
- [x] Document how to add a new provider (single page, step-by-step) (`docs/reference/PRICING_PROVIDER_REGISTRY.md`)
- [x] Add test: `set_price_provider()` override still works for test isolation (`tests/providers/test_provider_integration.py`)
