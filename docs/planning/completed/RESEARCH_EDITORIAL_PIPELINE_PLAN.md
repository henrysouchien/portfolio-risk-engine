...
**Depends on**:
- Plan #2 `docs/planning/completed/HANDOFF_ARTIFACT_V1_1_PLAN.md` — `HandoffArtifactV1_1` (Pydantic at `AI-excel-addin/schema/handoff.py:116`). **SHIPPED** per Codex R1 verification (commits `f69aa72`, `203ad57`, `5e47452`, `03a43ac`).
- Plan #1 `docs/planning/completed/THESIS_LIVING_ARTIFACT_PLAN.md` — `Thesis` (Pydantic at `AI-excel-addin/schema/thesis.py:222`). **SHIPPED** per Codex R1 verification (commit `5370a50` "feat(thesis): sub-phase A...").
- Plan #5 `docs/planning/completed/PROCESS_TEMPLATE_PLAN.md` — `ProcessTemplate` at `AI-excel-addin/schema/process_template.py:115`; uses `ProcessTemplate.investor_profile.strategy_bias` + `ProcessTemplate.section_config.required` (R2 fix — R1 cited the non-existent `required_sections`). **SHIPPED 2026-04-23** (master plan §12 row 5).
...
- **No qualitative-factor rollup, no thesis-coherence cross-check, no industry-context generator.** Phase 2. Industry context specifically belongs to Plan #7 `docs/planning/completed/INDUSTRY_RESEARCH_TOOLS_PLAN.md` — keeping it separate avoids two plans fighting over the same surface.
...
1. §12 row 10 status update: `completed/RESEARCH_EDITORIAL_PIPELINE_PLAN.md` → mark **(SHIPPED)** with date + commit refs (cross-repo: AI-excel-addin commits A–H + risk_module commit F + frontend commit G).
2. §9 footnote append (one paragraph): "The research-editorial pipeline anticipated above shipped 2026-MM-DD as plan #10 (`completed/RESEARCH_EDITORIAL_PIPELINE_PLAN.md`). v1 ships the deterministic three-generator pipeline + workspace UI section. LLM arbiter, markdown/PDF export, and editorial_memory coupling are Phase 2."
...
- `docs/planning/completed/RESEARCH_EDITORIAL_PIPELINE_PLAN.md` (this doc) — header status `DRAFT` → `SHIPPED` + ship summary.
...
| Industry-context generator (G5 surface) | Plan #7 `docs/planning/completed/INDUSTRY_RESEARCH_TOOLS_PLAN.md` owns G5. Two plans on same surface = scope fight. | Plan #7. |
...
| # | Plan | Status |
|---|---|---|
| 1 | THESIS_LIVING_ARTIFACT | SHIPPED 2026-04-22 |
| 2 | HANDOFF_ARTIFACT_V1_1 | SHIPPED 2026-04-22 |
| 3 | MODEL_BUILD_CONTEXT | SHIPPED 2026-04-22 |
...
| 7 | INDUSTRY_RESEARCH_TOOLS | SHIPPED 2026-04-28 |
...
V2.P9 status after plan #10 + Plan #7 merge: **11 SHIPPED / 0 DESIGNED**.
...
Send to Codex with:
- This file (`docs/planning/completed/RESEARCH_EDITORIAL_PIPELINE_PLAN.md`).
