...
**Status:** Draft v7 — folds in V4c R2 + V4c-tail SHIPPED findings (parallel session commit `5f61dd5b`, 2026-04-25): SnapTrade Connected User rate $2.00 → $1.50 (contracted dashboard rate, lower than public list); Manual Refresh `connections.refresh_brokerage_authorization` $0.05/call (was TBD); NEW per-call axis `accounts.orders` $0.002/call (Recent Orders dimension, moved out of subscription set).
