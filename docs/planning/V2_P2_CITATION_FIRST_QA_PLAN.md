...
**Status:** SLICES A+B+C+D SHIPPED + LIVE-VERIFIED 2026-05-06; F55+F56 closed. Slice E + Slice D-Persist remain as named follow-ups.
...
| **D** — Citation chips + sources + validator on chat surfaces | **SHIPPED + LIVE-VERIFIED 2026-05-06** | Plan: `completed/V2_P2_SLICE_D_PLAN.md` (R9 PASS after 9 Codex review rounds). Commits: `2bc2c6e7` chassis, `fdcfa4b2` hooks+store, `4c934f5b` components, `9f0050d4` ChatCore+ConversationFeed wiring, `556a01dd` TODO mark. | Scope expanded vs original spec to cover BOTH research and main chat surfaces (no new standalone page — single-anchor wiring at `ConversationFeed` + `ChatCore`). Side-channel attachment model keyed by `assistantMessageId` with atomic reducer + sequence guards survives `replaceMessages`, lifecycle mutations (delete/regenerate/retry), and refresh races. `remarkCitations` plugin emits custom HAST elements rendered via `<CitationChip>` component override. `<ValidationBanner>` implements 4-state spec (violations / hidden / validation_timeout / validator_internal_error) with edge precedence rules. 1236 frontend tests pass. Live MSFT 10-Q query rendered inline `[S15]` chips, 16-source `<SourcesList>` footer, and `validation_timeout` banner per spec. Slice D-Persist (server-side persistence) + Slice E remain as named follow-ups. |
