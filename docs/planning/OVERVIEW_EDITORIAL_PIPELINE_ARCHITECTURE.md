...
### Phase 2 (next)
- Events, Income, Trading, Factor, Tax Harvest generators
- Attention items UI rendering
- Dedicated onboarding wizard (if free-form analyst conversation proves insufficient)
- Revision/diff markers
- `complete_structured()` on `CompletionProvider`
- Architecture test for invalidation hooks
- Eval framework (predefined portfolio scenarios)
- Observability dashboards

### Phase 3+ (deferred)
- Engagement tracking
- Per-user generator activation
- Editorial_memory seeding from portfolio composition
