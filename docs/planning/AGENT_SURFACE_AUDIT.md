...
| **Phase 3D** | Option payoff + `OptionLeg`/`OptionStrategy` domain type extraction | 🟢 **ACTIONABLE — capability-unlock**. ~3-4 days. Current asymmetry: `portfolio_math.options` already has `black_scholes_price`/`_greeks`/`implied_volatility` locally, but `OptionStrategy`/`OptionLeg` + payoff math still HTTP-only. Extraction unlocks: local strategy screening (50 combos = ms not 15s), composition with local Black-Scholes for vol/time surfaces, `strategy_payoff(strategy, price_array)` composable with MC terminal-price arrays, custom price grids. Schedule against priority. |
