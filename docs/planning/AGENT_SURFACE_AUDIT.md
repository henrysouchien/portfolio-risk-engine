...
| **Phase 3D** | Option payoff + `OptionLeg`/`OptionStrategy` domain type extraction | ✅ **SHIPPED 2026-04-22** — see `PORTFOLIO_MATH_OPTIONS_PAYOFF_EXTRACTION_PLAN.md` ship log. `portfolio_math` now exports `OptionLeg`, `OptionStrategy`, and all 10 pure payoff functions at root, matching Phase 2's pricing-primitive pattern, with legacy `options.*` paths preserved via shims. |
