...
**BEFORE**: Monolithic `core/risk_orchestration.py` (896 lines) mixing CLI, business logic, and formatting
...
├── compat.py             # Backward-compatible wrappers for portfolio_risk_engine/data_loader.py
...
The `compat.py` module provides drop-in replacements for legacy `portfolio_risk_engine/data_loader.py` functions:
...
**Positions CLI** (`scripts/run_positions.py`):
```bash
# Basic usage - show all positions
python3 -m scripts.run_positions --user-email user@example.com

# Plaid only with consolidation
python3 -m scripts.run_positions --user-email user@example.com --source plaid --consolidated

# Export to JSON
python3 -m scripts.run_positions --user-email user@example.com --format json --output positions.json

# Chain to risk analysis
python3 -m scripts.run_positions --user-email user@example.com --consolidated --to-risk
```
...
# fmp-mcp (14 tools)
claude mcp add fmp-mcp -s user -- python3 -m fmp.server
...
**FMP MCP Server** (`fmp/server.py`, 14 tools):
...
**AI Function Registry** (`utils/ai_function_registry.py`):
...
- **CLI Users**: `python3 -m core.risk_orchestration --portfolio config/portfolio.yaml` → formatted text output
...
All primary analysis functions in `core/risk_orchestration.py` support both **CLI mode** (default) and **API mode** (`return_data=True`):
...
**CLI Usage:**
```bash
python3 -m core.risk_orchestration --portfolio config/portfolio.yaml
...
│  │ core/risk_orchestration.py     │  │ routes/         │  │ routes/      │ │
...
│  │ portfolio_risk_engine/risk_summary.py │  │ portfolio_risk_engine/factor_utils.py │  │ utils/       │ │
...
```
1. User Input
   ├── CLI: "python3 -m core.risk_orchestration --portfolio config/portfolio.yaml"
   ├── Direct API: "POST /api/direct/portfolio"
   └── AI: "Analyze my portfolio risk"
   
2. Routes Layer (Returns Result Objects)
   ├── run_portfolio() → RiskAnalysisResult in core/risk_orchestration.py
...
4. Data Layer (Supporting Functions)
   ├── build_portfolio_view() in portfolio_risk_engine/portfolio_risk.py
   ├── run_what_if_scenario() in portfolio_risk_engine/portfolio_optimizer.py
   └── get_stock_risk_profile() in portfolio_risk_engine/risk_summary.py
...
├── 🔧 utils/ai_function_registry.py         # Claude AI function registry (16 functions)
...
│   ├── 🤖 fmp/server.py                # fmp-mcp FastMCP server (14 tools)
...
│   ├── 🖥️ core/risk_orchestration.py                     # CLI interface with Result Objects support
│   ├── 🖥️ core/run_portfolio_risk.py           # Portfolio risk analysis helpers
│   ├── 🖥️ scripts/run_positions.py                # Positions CLI for brokerage data
│   ├── 🖥️ scripts/run_trading_analysis.py         # Trading analysis CLI (Plaid/SnapTrade/IBKR Flex)
│   ├── 🖥️ scripts/run_factor_intelligence.py      # Factor intelligence CLI
...
│   │   ├── realized_performance/aggregation.py # Transaction-based realized performance
...
│   ├── 💼 portfolio_risk_engine/portfolio_risk.py               # Legacy portfolio risk calculations
│   ├── 📈 portfolio_risk_engine/portfolio_risk_score.py         # Legacy risk scoring system
│   ├── 📊 portfolio_risk_engine/factor_utils.py                 # Factor analysis utilities
│   ├── 📋 portfolio_risk_engine/risk_summary.py                 # Legacy single-stock risk profiling
│   ├── ⚡ portfolio_risk_engine/portfolio_optimizer.py           # Legacy portfolio optimization
│   ├── 🔌 portfolio_risk_engine/data_loader.py                  # Data fetching and caching (uses fmp/compat.py)
...
│   ├── 🤖 utils/gpt_helpers.py                  # GPT integration utilities
│   ├── 🔧 core/proxy_builder.py                # Factor proxy generation
│   ├── 🏦 providers/plaid_loader.py                 # Plaid brokerage integration
│   ├── 💼 providers/snaptrade_loader.py             # SnapTrade brokerage integration
│   └── 🛠️ portfolio_risk_engine/risk_helpers.py                 # Risk calculation helpers
│
...
│       └── logs/                          # Production monitoring and alerts
│
├── 📁 Configuration Files
...
```python
# core/risk_orchestration.py (1217 lines)
...
```python
# core/risk_orchestration.py (Routes Layer)
...
### 1. Data Layer (`portfolio_risk_engine/data_loader.py`)
...
### 2. Factor Analysis (`portfolio_risk_engine/factor_utils.py`)
...
### 3. Portfolio Risk Engine (`portfolio_risk_engine/portfolio_risk.py`)
...
### 4. Portfolio Performance Engine (`portfolio_risk_engine/portfolio_risk.py`)
...
### 4. Single Stock Profiler (`portfolio_risk_engine/risk_summary.py`)
...
### 5. Data Quality Validation (`core/proxy_builder.py`)
...
**Portfolio Runner** (`core/run_portfolio_risk.py`):
- End-to-end portfolio analysis
- Configuration validation
- Error handling and reporting
- Output formatting

**Single Stock Runner** (`python3 -m core.risk_orchestration --stock`):
- Individual stock diagnostics via `run_stock()` function
- Factor model validation
- Detailed regression analysis

**Risk Runner** (`core/risk_orchestration.py`):
- Flexible risk analysis entry point
- What-if scenario testing
- Batch processing capabilities

**Trading Analysis Runner** (`scripts/run_trading_analysis.py`):
...
**Factor Intelligence Runner** (`scripts/run_factor_intelligence.py`):
...
### Plaid Financial Data Integration (`providers/plaid_loader.py`)
...
### SnapTrade Financial Data Integration (`providers/snaptrade_loader.py`)
...
2. **Single Stock Profile**:
   ```bash
   python3 -m core.risk_orchestration --stock AAPL --start 2020-01-01 --end 2023-12-31
   ```
...
| Caching System | `portfolio_risk_engine/data_loader.py` | ✅ Complete | Multi-level caching |
| Display Utils | `utils/helpers_display.py` | ✅ Working | Formatted output |
| Input Utils | `utils/helpers_input.py` | ✅ Working | Configuration parsing |
| Portfolio Optimization | `portfolio_risk_engine/portfolio_optimizer.py` | ✅ Working | Min variance and max return |
| GPT Integration | `utils/gpt_helpers.py` | ✅ Working | Peer generation and interpretation |
| Proxy Builder | `core/proxy_builder.py` | ✅ Working | Factor proxy generation |
...
| FMP MCP Server | `fmp/server.py` | ✅ Working | fmp-mcp: 14 tools for financial data |
| Position Service | `position_service.py` | ✅ Working | Multi-provider position orchestration |
| Positions CLI | `scripts/run_positions.py` | ✅ Working | CLI for brokerage position data |
...
| Realized Performance | `core/realized_performance/aggregation.py` | ✅ Working | Transaction-based realized P&L |
| Plaid Integration | `providers/plaid_loader.py` | ✅ Working | Financial data import |
| Risk Helpers | `portfolio_risk_engine/risk_helpers.py` | ✅ Working | Risk calculation utilities |
...
### Display Utilities (`utils/helpers_display.py`)
...
**Usage**:
```python
from utils.helpers_display import compare_risk_tables

# Compare before/after risk metrics
comparison = compare_risk_tables(old_risk_df, new_risk_df)
```

### Input Processing (`utils/helpers_input.py`)
...
### GPT Integration (`utils/gpt_helpers.py`)
